...
const botConnect = require("@whiskeysockets/baileys").default;
...
function smsg(conn, m, store) {
    ...
    if (m.key) {
        ...
        if (m.isGroup) m.participant = jidDecode(m.key.participant).user || "";
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.isBaileys = m.quoted.id
                ...
                : false;
            m.quoted.sender =
                jidDecode(m.msg.contextInfo.participant).user || "";
            m.quoted.fromMe = m.quoted.sender === jidDecode(conn.user.id).user;
            m.quoted.text =
                m.quoted.text ||
                m.quoted.caption ||
    ...
    m.copy = () => exports.smsg(conn, M.fromObject(m));
...
async function startBot() {
    ...
    client.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            let reason = new Boom.Boom(lastDisconnect?.error)?.output
                .statusCode;
            ...
            console.log("Bot Connected with the Server Successfully!");
        }
    });
